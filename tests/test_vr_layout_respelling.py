...
keeps for a monitor -- and a key the reader cannot find falls back to the
default, so a bare rename would have lost where he placed the main screen.  It is
rewritten at the startup that first reads it; the reader knows only "main".
"""
...
class TestTheHeadsetLayoutFile:
    ...
    a monitor -- and a key the reader cannot find falls back to the default,
    so a bare rename would have lost where he placed the main screen."""
    ...
    def test_a_file_already_in_todays_spelling_is_left_alone(self, tmp_path: Path):
        path = tmp_path / LAYOUT_FILENAME
        write_layout(path, DEFAULT_LAYOUT)
