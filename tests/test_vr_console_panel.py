...
def _published(mode: str, **fields) -> ConsoleModel:
    """The panel Fun Time publishes for *mode*, its buttons declared, the broker up."""
    return ConsoleModel(mode=mode, locked=False,
                        rows=console_rows(MainSlot(mode=mode, locked=False)),
...
def _live_console() -> ConsoleHud:
    ...
    return ConsoleHud(
        modes=ModeHud(video="scene one"),
        console=_published("video", osr2=OSR2_ROBOT_HAND),
        drive=_drive(),
    )
