...
from fun_time.windows_bridge_dashboard_bridge_app import build_parser, main
...
def test_main_dispatches_dashboard_snapshot_write(tmp_path: Path):
    ...
    with patch("fun_time.windows_bridge_dashboard_bridge_app.write_dashboard_snapshot", return_value=True) as write_snapshot:
        code = main([
            "--output-file",
            str(output),
            "--f-mode-enabled",
