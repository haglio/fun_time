...
class TestOrientationCorrection:
    """Landscape beats portrait for the primary role; ties go to the leftmost."""

    def test_landscape_primary_portrait_secondary_keeps_assignment(self):
        monitors = [_mi(0, 0, 2560, 1392), _mi(2560, 0, 1440, 3440)]
        primary, secondary = get_logical_monitor_rects(monitors, primary_index=1, secondary_index=2)

        assert primary == MonitorRect(0, 0, 2560, 1392)
        assert secondary == MonitorRect(2560, 0, 1440, 3440)

    def test_swapped_config_corrects_to_landscape_primary(self):
        monitors = [_mi(0, 0, 2560, 1392), _mi(2560, 0, 1440, 3440)]
        primary, secondary = get_logical_monitor_rects(monitors, primary_index=2, secondary_index=1)

        assert primary == MonitorRect(0, 0, 2560, 1392)
        assert secondary == MonitorRect(2560, 0, 1440, 3440)

    def test_both_landscape_falls_back_to_leftmost_primary(self):
        monitors = [_mi(1920, 0, 1920, 1080), _mi(0, 0, 2560, 1440)]
        primary, secondary = get_logical_monitor_rects(monitors, primary_index=1, secondary_index=2)

        assert primary == MonitorRect(0, 0, 2560, 1440)
        assert secondary == MonitorRect(1920, 0, 1920, 1080)

    def test_both_portrait_falls_back_to_leftmost_primary(self):
        monitors = [_mi(1440, 0, 1080, 1920), _mi(0, 0, 1440, 2560)]
        primary, secondary = get_logical_monitor_rects(monitors, primary_index=1, secondary_index=2)

        assert primary == MonitorRect(0, 0, 1440, 2560)
        assert secondary == MonitorRect(1440, 0, 1080, 1920)

    def test_single_monitor_uses_same_for_both(self):
        monitors = [_mi(0, 0, 2560, 1440)]
        primary, secondary = get_logical_monitor_rects(monitors, primary_index=1, secondary_index=1)
    ...
    def test_index_clamped_to_available_monitors(self):
        monitors = [_mi(0, 0, 2560, 1440)]
        primary, secondary = get_logical_monitor_rects(monitors, primary_index=1, secondary_index=5)
