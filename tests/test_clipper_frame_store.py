...
from fun_time.robot_hand.clipper.frame_store import (
    ensure_loaded,
    load_range,
    safe_frame,
    signature_for_index,
...
def test_ensure_loaded_expands_missing_edges_and_bumps_render_rev():
    ...
    with patch("fun_time.robot_hand.clipper.frame_store.load_range", side_effect=[left_frames, right_frames]) as load:
...
def test_ensure_loaded_is_noop_when_requested_range_is_already_loaded():
    ...
    with patch("fun_time.robot_hand.clipper.frame_store.load_range") as load:
...
def test_safe_frame_loads_missing_index_on_demand():
    ...
    with patch("fun_time.robot_hand.clipper.frame_store.ensure_loaded", side_effect=fake_ensure_loaded) as ensure:
...
def test_safe_frame_raises_when_on_demand_load_still_fails():
    ...
    with patch("fun_time.robot_hand.clipper.frame_store.ensure_loaded"):
...
def test_signature_for_index_caches_processed_signature():
    ...
    with patch("fun_time.robot_hand.clipper.frame_store.preprocess_frame_signature", return_value=cached_signature) as preprocess:
