...
def _watched(meta: dict | None, completions: int, skips: int, locks: int, weight: float) -> dict:
    """What Evolver stamps: the counts summed from every app, and their weight."""
    return {
        **(meta or {}),
        "watch": {"completions": completions, "skips": skips, "locks": locks, "weight": weight},
    }


def test_build_breeding_rows_ranks_by_the_stamped_weight_with_metadata_identity(tmp_path: Path):
    media_root, metadata_root, paths = _library(tmp_path, {
        "loved": _watched(_meta("Alpha", "111"), 6, 0, 1, 8.0),
        "wide_meh": _watched(_meta("Dancing", "222"), 1, 1, 0, 1.0),
        "hated_bare": _watched(None, 0, 9, 0, 0.125),
        "unwatched": _meta("Alpha", "333"),
        "no_metadata": None,
    })
...
def test_render_breeding_report_limits_rows_and_handles_empty():
    rows = sorted(
        (
            BreedingRow(
                path=f"c:\\clips\\clip{i:02}.mp4", weight=1.0 + i, completions=i, skips=0,
                locks=0, orientation="?", action="", seed="", prompt="",
            )
            for i in range(20)
        ),
...
def test_main_prints_the_leaderboard_for_the_configured_library(cfg_factory, tmp_path, capsys):
    ...
    assert "unwatched.mp4" not in out
