...
def test_main_apply_enter_runs_non_window_omnipause_side_effects(tmp_path: Path):
    ...
    with (
        patch("fun_time.controller_omnipause_app.vlc_http_cmd", return_value=True) as vlc_cmd,
        patch("fun_time.controller_omnipause_app.ensure_playback_state", return_value=True) as ensure_playback,
    ):
        code = main([
            "apply-enter",
            "--omni-paused",
