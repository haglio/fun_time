...
class TestPaceRows:
    ...
    def test_the_rate_pair_says_what_a_satellites_rate_pair_says(self):
        ...
        rendered = HudRenderer("landscape").render(
            HudModel(side="landscape", playback_speed=1.0))
        satellite = [button.tooltip for _rect, button in rendered.targets.buttons
                     if button.action.startswith("landscape_speed_")]
        console = [button.tooltip for row in console_rows(MainSlot(mode="video"))
                   for button in row if button.action.startswith("main_player_speed_")]
