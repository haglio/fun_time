...
def _runner(tmp_path: Path) -> DispatchLoopRunner:
    ...
    The flash tests used to graft _flash_main_player_notice onto a two-attribute
    ...
    config = BridgeConfig(
        **{name: tmp_path / f"{name}.txt" for name in path_fields},
        ...
        portrait_sources="",
        landscape_sources="",
        main_player_notice_file=tmp_path / "main_player_notice.txt",
    )
    return DispatchLoopRunner(
        ...
        windows=WindowRoles(pids=ChildPids(main_player=MAIN_PLAYER_PID)),
...
class TestReadMainPlayerNotice:
    ...
    def test_malformed_sequence_is_empty(self, tmp_path):
        ...
        assert read_main_player_notice(path) == (0, "", "")
