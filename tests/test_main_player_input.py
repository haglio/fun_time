"""What arrives in the main player's window, and what it is taken to mean.

The translation itself: which SDL event reaches the pointer and which is the
window being closed -- and that a key reaches nothing, the room's keyboard
being Fun Time's.  Until the dealer was pulled out of the run loop no test fed
a pygame event to anything, so the mapping from ``ev.button`` /
``ev.buttons`` / ``ev.pos`` to a collaborator's call was unpinned, including the
two places a wrong index quietly turns a drag into a hover.

What each collaborator then DOES is theirs, and is tested in
test_main_player_pointer and test_main_player_dashboard.
"""
...
def _input() -> tuple[Input, SpyPointer, SpyDashboard]:
    pointer, dashboard = SpyPointer(), SpyDashboard()
    return Input(pointer, dashboard), pointer, dashboard
...
class TestTheWindowBeingClosed:
    def test_a_quit_gesture_is_asked_of_the_session_rather_than_taken(self):
        ...
        that goes, not this one window out of six -- see main_player.dashboard."""
        window_input, _p, dashboard = _input()
...
class TestTheMouse:
    def test_a_left_press_lands_at_its_place_in_this_window(self):
        ...
        window_input, pointer, _d = _input()
...
class TestTheKeyboard:
    def test_a_key_pressed_on_the_window_reaches_nothing(self):
        """Fun Time's hotkeys are the keyboard for the whole room.  Keys this
        window answered itself were the main player's standalone ones, and while
        OmniPause has the room's keys switched off they went through to a room
        that is supposed to hear nothing."""
        window_input, pointer, dashboard = _input()
        ...
        _deal(window_input,
              ...
              SimpleNamespace(type=pygame.KEYDOWN, key=pygame.K_r, mod=0),
              SimpleNamespace(type=pygame.KEYUP, key=pygame.K_r, mod=0))

        assert (pointer.calls, dashboard.asked) == ([], 0)
