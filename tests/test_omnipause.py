...
def test_toggle_enters_omnipause_when_not_already_paused():
    plan = build_omnipause_plan("toggle", omni_paused=False, primary_mode="nau")
...
def test_toggle_leaves_omnipause_when_already_paused():
    plan = build_omnipause_plan("toggle", omni_paused=True, primary_mode="genau")
...
def test_leave_nau_mode_resumes_nau():
    plan = build_omnipause_plan("leave", omni_paused=True, primary_mode="nau")
...
def test_leave_hybrid_mode_resumes_nau():
    # Hybrid displays Nau (Genau only drives the OSR2), so leaving omnipause
    # resumes Nau, not any separate primary player.
    plan = build_omnipause_plan("leave", omni_paused=True, primary_mode="hybrid")
