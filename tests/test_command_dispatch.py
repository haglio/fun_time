...
def _publish_drive(config: BridgeConfig, *, amplitude: int) -> None:
    ...
    text += "".join(f"{f}=0\n" for f in flags) + "shape=sine\n"
    config.genau_drive_file.parent.mkdir(parents=True, exist_ok=True)
    config.genau_drive_file.write_text(text, encoding="utf-8")
...
def _set_current(config: BridgeConfig, player: Player, video: str, *, locked: bool = False,
...
def _fmode_result(**kwargs):
    ...
    return type("R", (), {
        "players": players,
        "enabled": enabled,
        "log_message": f"F-mode {'enabled' if enabled else 'disabled'}: "
                       f"{', '.join(player.label for player in players)}",
    })()
...
def test_a_named_player_goes_into_f_mode_alone(tmp_path: Path, player: Player):
    """What each HUD's own F button posts: that player and no other."""
    config = _make_config(tmp_path)

    new_state, _ops, mock_fmode = _dispatch_fmode(
        f"{player.label}_fmode", _make_state(), config)

    assert list(_f_modes(new_state)).count(True) == 1
    assert _f_modes(new_state)[list(Player).index(player)] is True
...
def test_reset_still_fires_for_any_one_thing_left_narrowing_the_side(
    ...
    with patch("fun_time.command_dispatch.apply_satellite_filter") as mock_filter:
        mock_filter.return_value = _filter_result(count=10)
        _state, ops = dispatch_command(
            "portrait_reset", _make_state(portrait=narrowed), config)
...
def test_the_bare_nudge_reaches_the_video_while_the_hand_is_held(tmp_path: Path):
    ...
    dispatch_command("speed_up", _make_state(main_mode=MainMode.VIDEO), config)
...
def test_speed_min_and_max_route_to_the_active_engine(tmp_path: Path):
    config = _make_config(tmp_path)
    dispatch_command("speed_min", _make_state(main_mode=MainMode.VIDEO), config)
    assert config.main_player_cmd_file.read_text(encoding="utf-8") == "SET_SPEED min\n"

    config = _make_config(tmp_path)
    dispatch_command("speed_max", _make_state(main_mode=MainMode.GENAU), config)
...
def test_speed_max_sets_main_player_video_in_video_mode(tmp_path: Path):
    config = _make_config(tmp_path)
    _set_main_player_driving(config, driving=False)
    dispatch_command("speed_max", _make_state(main_mode=MainMode.VIDEO), config)
...
def test_next_in_a_loop_of_several_clips_moves_on_within_the_loop(tmp_path: Path):
    ...
    assert _cmds(config, 2) == ["NEXT"]
    assert state.portrait.loop == "seed"
...
def test_a_main_reorder_reaches_genau_when_genau_is_the_one_showing(tmp_path, monkeypatch):
    ...
    assert config.genau_cmd_file.read_text(encoding="utf-8").split() == ["LATEST", "SHUFFLE"]
    assert calls == []
...
def test_main_reset_drops_the_length_mode_and_f_mode_together(tmp_path, monkeypatch):
    ...
    state, ops = dispatch_command("main_reset", _make_state(main_scripted_filter=True), config)
...
def test_main_reset_does_not_reshuffle_a_player_that_was_not_narrowed(tmp_path, monkeypatch):
    ...
    dispatch_command("main_reset", _make_state(main_scripted_filter=False), config)
...
class TestOrigeneratorTransport:
    ...
    def test_no_filter_reaches_the_hosted_show_as_clear_filter(self, tmp_path):
        ...
        config = _origenerator_config(tmp_path)
        state = BridgeState(portrait=SatelliteState(filter="alpha"), satellites_mode="origenerator")
...
def test_discard_with_no_clip_on_screen_announces_nothing(tmp_path: Path):
    ...
    _state, ops = _discard(2, _make_state(portrait=SatelliteState(locked=False)), config)
