...
- **From a source image** → opens `example.com/create`, fills the positive + negative image prompts, and pins a floating note with the **video prompt** and original settings — so you regenerate the image first, then make the video from it. The video prompt/settings are also **stashed**: once you generate the image and click Provider's own "make video" button (which carries the image over as the base — the userscript can't set a base image itself), the note reappears on the video page and the video prompt + settings auto-fill, the same as a text-to-video regen. The note's copy button stays as a fallback. The stash lives in `sessionStorage` (per-tab, so it survives Provider's in-app navigation and a reload) and expires after 30 min; an explicit text-to-video regen supersedes it.
