...
SyncRobotHandState() {
    ...
    args := "sync-robot-hand"
        ...
        . " --password " . Q(VLC_PASS)
    if (RunWindowsBridgeRuntimeFlowAction(args) != 0)
        return
    plan := LoadWindowsBridgeRuntimeFlowResult(resultPath)
    if !IsObject(plan)
        return
...
ToggleFMode() {
    ...
    if (RunWindowsBridgeRuntimeFlowAction(args) != 0)
        return
    result := LoadWindowsBridgeRuntimeFlowResult(resultPath)
    if !IsObject(result)
        return
    if !result["success"] {
...
StartWindowsBridge() {
    ...
    global ROBOT_HAND_PY, ROBOT_HAND_MODULE, DASHBOARD_MODULE, WINDOWS_BRIDGE_MANIFEST_PATH, CONFIG_PATH, ROBOT_HAND_CLIPS
    ...
    args := "launch-ui-companions"
        ...
        . " --windows-bridge-manifest-path " . Q(WINDOWS_BRIDGE_MANIFEST_PATH)
