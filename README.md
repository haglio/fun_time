...
Rebuilding is therefore a deliberate gesture: **Shuffle** (reshuffle), **Latest** (spoken, newest-first), a filter, a reset, or an F-Mode toggle. Those are also where videos added since the last session first appear.
...
During satellite builds, each action group **collapses to one playlist slot**, so the same subject+scene doesn't recur once per action. Shuffled builds draw that member weighted by the watch stats below; Latest (newest-first) instead keeps the group's newest member and orders by recency. Either way it's one entry per group. Videos without a metadata sidecar behave exactly as before.
