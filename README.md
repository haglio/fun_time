...
Everything Origenerator answers to on its own microphone can be said to a region here, because the session owns the room's mic for both of them: it hears the phrase and posts the words on the hosted app's channel, which matches them with its own vocabulary. Name the side, then say what you would have said to Origenerator — a shelf to play ("portrait favorites", "landscape experiments", also requests and trash), the show's controls ("landscape play slideshow", "portrait stop slideshow" — play/start/open, pause, stop/end/close), a targeted fix ("portrait fix teeth", "fix all"), "enhance" for the better version of the picture on that region, or "go now" to animate it as a Genau clip. Naming a side makes it the active one, so every phrase but "go now" then drops the side word and reaches that region, exactly as a bare "lock" or "next" reaches the player last addressed; bare "go now" stays the switch into Genau mode. The parts a fix can name beyond the anatomy every picture has are library vocabulary, so they come from `content.local.json`'s `origenerator_fix_parts` and have to be spelled the way Origenerator's own overlay spells them. A spoken request — "landscape request no feet over" — reaches a region the same way, but it cannot ride the grammar: its words are the speaker's own, so no list can hold them. It rides instead the free recognizer that already runs alongside the grammar one to caption an unrecognized phrase, and `fun_time.spoken_request` decides which of those transcriptions belong to a request: it opens on the lead word, forwards every utterance verbatim (markers and all — pause as often as you like, and nothing said between them is read as a command), and gives up after eight if the "over" is never heard. Each is posted on the same `*_SAY:` verb every other phrase uses, because assembling them into a request is the hosted app's own dictation's business — it owns the markers and the picture the request is about. Fun Time only collects one while Origenerator is actually showing, since outside that mode "request" is not a command and opening one would swallow the next several things said to the room.
...
**A reset does nothing to a player that is already in one.** Every rebuild behind it reshuffles and lands on the new list's first entry, so a press with nothing to put back was the quickest way to keep changing what was playing — press it three times and see three videos. A satellite is skipped whole when none of the things a reset takes off is set (`_RESET_STATE_FIELDS` is the one list both the clearing and the check read, so a narrowing cannot be cleared by one and missed by the other); the main player's half is Nau's, which ignores a length mode it is already running unless a compilation is what needs leaving.
...
`PORTRAIT_SAY:<words>` for something the session heard said about one region —
a phrase off the recognizer's grammar ("favorites", "fix teeth", "enhance") or
one utterance of a spoken request, which is the same verb carrying free words.
The paused flag freezes its shows — and its own looping thumbnails —
