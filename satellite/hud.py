...
def label_is_filtered(label: str, filter_query: str) -> bool:
    ...
    Theta" row are clips it keeps, while a plain "redacted" row is *not* kept by a
    "redacted, redacted insertion" filter and must not read as though it were.
...
# Words the metadata writes in front of an act ("POV redacted") that qualify how it
...
# line "POV redacted" both words went white under a "redacted" filter, which said the
