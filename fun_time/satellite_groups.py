...
def is_single_video_loop(which: int, state: BridgeState, config: BridgeConfig) -> bool:
    return bool(state.side(which).loop) and (
        read_satellite_status(config.side(which).status_file).playlist_length == 1)
