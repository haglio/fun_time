...
Hand's motion.  Why each ordering is what it is, is pinned in
...
class MotionDials:
    """The Robot Hand's motion as it stood, and as it can be asked for again."""
    cruise: bool
    speed: int
    amplitude: int
    center: int
...
def held(dials: MotionDials) -> bool:
    """Whether the motion is stilled: a hold is an amplitude of zero, at
...
def dials_text(dials: MotionDials) -> str:  # the shape Genau's own files use
    return (
...
def parse_dials(text: str) -> MotionDials | None:  # None as an absent file does
    values = dict(
