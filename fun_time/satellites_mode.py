"""The satellite side's two modes: player and origenerator.
...
side shows.  In ``player`` mode it is the session as ever: the Random Favs
...
Origenerator process runs for the whole session (parked while in player mode),
so a switch is minimize/restore plus the topmost restack, the way nau<->genau
...
STARTUP_SATELLITES_MODE = PLAYER_MODE
...
def toggled_satellites_mode(satellites_mode: str) -> str:
    ...
    return PLAYER_MODE if origenerator_shows(satellites_mode) else ORIGENERATOR_MODE
