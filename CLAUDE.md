...
A finished run appends one row to `docs/integration-runs.md` — UTC time, the fun_time and `player_core` HEADs it ran, counts, and the args if you narrowed it. Commit that row with your branch: there is no CI for this suite, so the file is the repo's only record that it ever passed.
